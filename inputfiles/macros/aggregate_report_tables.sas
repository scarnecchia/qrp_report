	...
	%put =====> MACRO CALLED: aggregate_report_tables;
	    ...
	    %if %str("&reporttype") = %str("T1") %then %do;
			%if %index(&datasetlist.,t1cida) > 0 %then %do;
			  %agg_report(infile=t1_cida, outfile=agg_t1cida, name=group, where=%nrstr(lowcase(group) in (&&grouplist_&n..))); 
			%end;
	    ...
	    %if %str("&reporttype") = %str("T2L1") %then %do;
			%if %index(&datasetlist.,t2cida) > 0 %then %do;
			  %agg_report(infile=t2_cida, outfile=agg_t2cida, name=group, where=%nrstr(lowcase(group) in (&&grouplist_&n..)));
			%end;
			...
			%if %index(&datasetlist.,t2conc) > 0 %then %do;
			  %agg_report(infile=t2_concomitance, outfile=agg_t2conc, name=analysisgrp, where=%nrstr(lowcase(analysisgrp) in (&&grouplist_&n..))); 
			%end;
