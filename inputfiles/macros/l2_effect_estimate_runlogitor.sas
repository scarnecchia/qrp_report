...
%macro l2_effect_estimate_runlogitor(individualreturn =, 
    ...
    %if "&individualreturn" = "N" & %index(%lowcase(&customizecolumns.),redactevents) = 0 %then %do;
        data _sub1;
    ...
    %if "&individualreturn" = "Y" & %index(%lowcase(&customizecolumns.),redactevents) = 0 %then %do;
        data _forest;
