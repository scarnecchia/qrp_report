...
***************************************************************************************************;
    ...
    %do periodid = %eval(&look_start.) %to %eval(&look_end.);
		...
		%if (%index(&reporttype,L2) | %index(&reporttype,TREE)) and %index(&figurelist,F1) %then %do;
			%l2_psdistribution_createdata;
		%end;
        %if (%index(&reporttype,L2) | %index(&reporttype,TREE)) and %index(&figurelist,F2) %then %do;
