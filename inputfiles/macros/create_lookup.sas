 ...
 /* Define libname for location of templatefiles folder */
  ...
  %macro create_lookup();
     data lookup.lookup_footnotes_baseline;
	   ...
	   order = 10; description = "With variable ratio matching, each exposed subject is matched to a variable number of comparator subjects. The weight for each treated subject equals 1. The weight for each control subject equals the inverse of the matching ratio for that specific matched set."; output;
     ...
     data lookup.lookup_footnotes_censortables;
	   ...
	   order = 2;  description = "Time to end of observable data is for characterization purposes only. It does not necessarily represent at-risk time, and does not consider episode end, outcome occurrence, blackout period, or delay risk period start."; output;
	   order = 3;  description = "Represents episodes censored due to end of the exposure episode. In as-treated analyses, exposure episodes are defined using days supplied as recorded in outpatient pharmacy dispensing records, and episodes end after days supplied are exhausted or a pre-determined maximum episode duration is met. In point exposure analyses, exposure episodes end when a pre-determined maximum episode duration is met."; output;
	   order = 4;  description = "Represents episodes censored due to occurrence of request-defined event."; output;
	   order = 5;  description = "Represents episodes censored due to occurrence of additional user-defined criteria using drug, procedure, diagnosis, and/or laboratory codes."; output;
	   order = 6;  description = "Represents episodes censored due to evidence of death. Death data source and completeness varies by Data Partner."; output;
	   order = 7;  description = 'Represents episodes censored due to disenrollment from health plan. Data Partners often artificially assign a "disenrollment" date equal to data end date for members still enrolled on that date. Therefore, a patient may have dual reasons for censoring as "disenrollment" and "end of data" on the same day - this can be interpreted as right-censoring in most cases.'; output;
	   ...
	   order = 9;  description = "Represents episodes censored due to user-specified study end date."; output;
	 run; 
