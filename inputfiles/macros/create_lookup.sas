 ...
 /* Define libname for location of templatefiles folder */
  ...
  %macro create_lookup();
     data lookup.lookup_footnotes;
      ...
      type = "type4"; order = -1;  description = "The non-pregnant cohort includes patients who had neither live nor non-live birth outcomes (including live-birth delivery, stillbirth, ectopic pregnancy, spontaneous abortion, induced abortion, or trophoblastic disease), met all inclusion/exclusion criteria and were the same integer age on the last date of the matched pregnant patient's pregnancy episode."; output;
      ...
      type = "baseline";  order = 16; description = 'Includes members classified as having an unknown race by the Data Partner and patients in race categories where the total member count is between one and ten.'; output;
	   ...
	   type = "baseline"; order = 25; description = "Sun, J. W., Bourgeois, F. T., Haneuse, S., Hernández-Díaz, S., Landon, J. E., Bateman, B. T., Huybrechts, K. F. Development and validation of a pediatric comorbidity index. American Journal of Epidemiology. 2020;190(5):918-927."; output;
	   type = "baseline"; order = 26; description = "Pisters R, Lane DA, Nieuwlaat R, de Vos CB, Crijns HJGM, Lip GYH. A novel user-friendly score (HAS-BLED) to assess 1-year risk of major bleeding in patients with atrial fibrillation. Chest. 2010;138(5):1093-1100."; output;
	   type = "baseline"; order = 27; description = "Lip GYH, Nieuwlaat R, Pisters R, Lane DA, Crijns HJGM. Refining clinical risk stratification for predicting stroke and thromboembolism in atrial fibrillation using a novel risk factor-based approach. Chest. 2010;137(2): 263-272."; output;
	  ...
	  type = "t4l1moi"; order = 6;  description = "Assessing medical product use in the second trimester requires a pregnancy to continue to the second trimester (i.e. pregnancy was not terminated in the first trimester); the denominator is the total number of pregnancy episodes that progress to the second trimester."; output;
	  type = "t4l1moi"; order = 7;  description = "Assessing medical product use in the third trimester requires a pregnancy to continue to the third trimester. The denominator is the total number of pregnancy episodes that progress to the third trimester."; output;
     ...
     data lookup.lookup_attrition;
	   attrib claim_level   length = $10	format = $10.
		...
		claim_level = "Member";  descr = "Exclusion - Members must have at least one pregnancy outcome during the query period"; report_descr = "Had a pregnancy outcome claim during the query period"; output;
		...
		claim_level = "Episode"; descr = "Exclusion - Pregnancy episodes must be during the query period"; report_descr = "Pregnancy outcome during the query period sufficing user-specified incidence criteria"; output;
		...
		claim_level = "Episode"; descr = "Exclusion - Pregnancy episodes must satisfy the enrollment criterion"; report_descr = "Had sufficient continuous enrollment prior to pregnancy outcome"; output;
		...
		claim_level = "MIL"; descr = "Exclusion - Pregnancy episodes must satisfy the age range condition within the query period"; report_descr = "Pregnancy outcome recorded during specified age range evaluated at specified index date"; output;
