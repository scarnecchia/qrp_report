            ...
            %else %if &tablesub ne overall %then %do;
                ...
                %else %if %eval(&stratnum.=2) %then %do;
                    if missing(&firststrat.)=0 then do;
                        ...
                        %assignlabelvars(format=strip(put(&firststrat., mn_name.)), sortorder1 = month, sortorder2=);
                        %end;
                        %else %if &firststrat. = quarter %then %do;
                        %assignlabelvars(format=strip(put(&firststrat., qtr_name.)), sortorder1 = quarter, sortorder2=);
                        %end;
                        %else %do;
					...
					if missing(&firststrat.) = 0 and missing(&secondstrat.) =0 then do;
                        ...
                        %assignlabelvars(format=strip(put(&secondstrat., mn_name.)), sortorder1 = , sortorder2=month);
                        %end;
                        %else %if &secondstrat. = quarter %then %do;
                        %assignlabelvars(format=strip(put(&secondstrat., qtr_name.)), sortorder1 = , sortorder2=quarter);
                        %end;
                        %else %do;
