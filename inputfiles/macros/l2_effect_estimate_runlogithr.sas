    ...
    %if %eval(&NOBS.>=2) & %index(&customizecolumns.,events) = 0 %then %do;
        ...
        *format output dataset;
        data est; 
  			...
  			format analysisgrp subgroup $40. catnum best. HR_coef LowerWaldCL UpperWaldCL MonitoringPeriod 2.0;
            ...
            label UCL = "95% UCL";
  			...
  			keep analysisgrp subgroup catnum analysis subgroupcat MonitoringPeriod HR_95CI HR LCL UCL HR_pvalue HR_coef HR_se;
  		run;
	%end;
	%else %if %index(&customizecolumns.,events) > 0 %then %do;
		data est;
		  		format analysisgrp subgroup $40. catnum best.;
