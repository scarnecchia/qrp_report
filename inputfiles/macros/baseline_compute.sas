               ...
               /*Aggregate weights*/
                ...
                if lowcase(vartype) = 'dichotomous' then do;
                    ...
                    if metvar = 'PATIENT' then do;
                        ...
                        %end;
                        %else %if %sysfunc(prxmatch(m/T1|T2L1|T5/i,&reporttype.)) %then %do;
                        exp_std&i._char = ' ';
                        comp_std&i._char = ' ';
                        %end;
                    end;
                    if missing(exp_mean&i) or exp_mean&i = 0  then do;
                        ...
                        if total_exp_patients = 1 or total_exp_episodes = 1 then exp_std&i._char = 'NaN';
                    end;
                    %if "&includecomp" = "Y" %then %do;
                    ...
                    if missing(comp_mean&i) or comp_mean&i = 0  then do;
                        ...
                        if total_comp_patients = 1 or total_comp_episodes = 1 then comp_std&i._char = 'NaN';
                    end;
                    %end;
                end;
                else do;
                    if missing(exp_mean&i) then exp_mean&i = 0;
                    if missing(exp_std&i) then exp_std&i = 0;
                    ...
                    if missing(comp_std&i) then comp_std&i = 0;
                ...
                if lowcase(vartype) = 'dichotomous' then do;
                    ...
                    ** Calculate aggregated percent: 
                    ...
                    if index(metvar, 'SEX') >0 | index(metvar, 'RACE') >0 | index(metvar, 'HISPANIC') >0 then do;
                        ...
                        if missing(exp_std0) then exp_std0 = 0; 
                        ...
                        if missing(comp_std0) then comp_std0=0;
						...
						%else %if %str("&reporttype") = %str("T6") and &switch_count > 0  %then %do;
						  ...
						  if metvar = 'N_EPISODES' then do;
                            ...
                            if missing(exp_mean0) then exp_std0_char = '0';
						    %if "&stratifybydp" = "Y" %then %do;
						      %do nu_d = 1 %to &num_dp.;
                                ...
                                if exp_mean&nu_d. = 0 then exp_std&nu_d._char = '0';
							  %end;
                            %end;
						  end;
						%end;
                ...
                if lowcase(vartype) = 'continuous' and metvar ne 'MAHALANOBIS' then do;
                    ...
                    if missing(exp_mean0) then exp_mean0 = 0;
                    ...
                    if missing(comp_mean0) then comp_mean0 = 0;
