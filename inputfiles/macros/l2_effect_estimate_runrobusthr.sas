    ...
    *Determine at least 1 event in exposure and reference group;
    ...
    %if %eval(&nobs.>=1) %then %do;
        ...
        *Determine at least 1 event in exposure and reference group;
        ...
        %if %eval(&sumec.>0) & %eval(&SumSquareUnEC.>0) & %index(%sysfunc(transtrn(%lowcase(&redactcolumns.),sumevents,)),events) = 0 %then %do;
            ...
            *format output dataset;
            ...
            run;        
        %end;
        %else %if %index(%sysfunc(transtrn(%lowcase(&redactcolumns.),sumevents,)),events) > 0 %then %do;
        data est;
